...
// const exportCounterBtn eliminado
...
function rgbaForClass(name) {
  ...
  if (wrbColors[key]) return wrbColors[key];
  if (usdaColors[key]) return usdaColors[key];

  // Fallback estable para clases no mapeadas explícitamente.
  const fallback = ['#7f8c8d', '#8e44ad', '#0f766e', '#4f46e5', '#b45309'];
  const idx = Math.abs(Array.from(key).reduce((acc, ch) => acc + ch.charCodeAt(0), 0)) % fallback.length;
  return fallback[idx];
}
...
function initDashboard() {
  ...
  provenanceBox.innerHTML = `
    <b>Trazabilidad USDA:</b> puntos tomados de <b>${provenance.source_csv || 'sensor multinivel_unified_labeled_ready.csv'}</b>.
