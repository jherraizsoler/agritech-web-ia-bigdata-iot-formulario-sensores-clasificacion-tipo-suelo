# Proyecto | sensor multinivel Soil Intelligence

<p align="center">
  <img src="logos/logo_proyecto.svg" alt="Proyecto · equipo investigador" height="100" />
</p>

Proyecto de investigacion aplicada para detectar y caracterizar tipo de suelo a partir de telemetria multinivel sensor multinivel, con pipeline reproducible en Python, datasets listos para Weka y soporte visual para analisis tecnico.
...
- Es posible discriminar tipo de suelo con senal sensor multinivel multinivel real, sin instrumentacion adicional en cada muestra.
...
- Dataset exploratorio sensor multinivel: 282089 filas, 41 columnas.
...
Proyecto Proyecto 1/
  ...
  doc/
    ...
    sensor multinivel_weka_metodologia.md
    memoria_proyecto.md
  presentacion/
    index.html
    dashboard.html
  ...
  results/
    ...
    figures/
  build_sensor multinivel_weka_datasets.py
  verify_sensor multinivel_data.py
  diagnose_sensor multinivel_pipeline.py
  sensor multinivel_sensor_only_weka.csv
  sensor multinivel_wrb_classify_ready.csv
  sensor multinivel_wrb_classify_ready.arff
  sensor multinivel_usda_regression_ready.csv
  sensor multinivel_unified_labeled_ready.csv
  requirements.txt
```
...
### 2) Extraer telemetria sensor multinivel limpia

```powershell
python verify_sensor multinivel_data.py
```
...
```powershell
python build_sensor multinivel_weka_datasets.py
```
...
```powershell
python diagnose_sensor multinivel_pipeline.py
```
...
> Ejecutado con `train_sensor multinivel_models.py` usando **scikit-learn** (Python).  
...
```powershell
python train_sensor multinivel_models.py
```
